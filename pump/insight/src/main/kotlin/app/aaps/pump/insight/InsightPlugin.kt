        ...
        connectionService?.let { service ->
            ...
            if (insulin > 0) {
                try {
                    ...
                    rxBus.send(EventOverviewBolusProgress(rh, ch, PumpInsulin(0.0), id = detailedBolusInfo.id))
                    var trials = 0
                    ...
                    while (!bolusCancelled) {
                        ...
                        if (activeBolus != null) {
                            trials = -1
                            rxBus.send(EventOverviewBolusProgress(rh, ch, delivered = PumpInsulin(activeBolus.initialAmount - activeBolus.remainingAmount), id = detailedBolusInfo.id))
                        } else {
                            synchronized(_bolusLock) {
                                if (bolusCancelled || trials == -1 || trials++ >= 5) {
                                    if (!bolusCancelled) {
                                        rxBus.send(EventOverviewBolusProgress(rh, ch, delivered = PumpInsulin(insulin), id = detailedBolusInfo.id))
                                    }
                                }
                            }
